namespace Asteroids_TDD_ECS.Editor.Tests
{
    ...
    public class MovementTests : ECSTestFixture
    {
        ...
        protected override void SetUp()
        {
            ...
            _manager.SetComponentData( m_entity, new MovementInput{ Value = 0 } );
            _manager.SetComponentData( m_entity, new MovementSpeed{ Value = 1 } );
            _manager.SetComponentData( m_entity, new DeltaTime{ Value = 0.01f } );
            _manager.SetComponentData( m_entity, new PhysicsVelocity{ Linear = float3.zero } );
        ...
        public void VelocityDoesNotChange_When_MovementInput0()
        {
            ...
            float3 results = _manager.GetComponentData<PhysicsVelocity>( m_entity ).Linear;
        ...
        {
            _manager.SetComponentData( m_entity, new MovementInput{ Value = input } );
            _manager.SetComponentData( m_entity, new MovementSpeed{ Value = 1 } );
            _manager.SetComponentData( m_entity, new DeltaTime{ Value = time } );
            _manager.SetComponentData( m_entity, new PhysicsVelocity{ Linear = float3.zero } );
            _manager.SetComponentData( m_entity, new Rotation{ Value = quaternion.identity } );
        ...
        {
            _manager.SetComponentData( m_entity, new MovementInput{ Value = 1 } );
            _manager.SetComponentData( m_entity, new MovementSpeed{ Value = 1 } );
            _manager.SetComponentData( m_entity, new DeltaTime{ Value = 0.01f } );
            _manager.SetComponentData( m_entity, new PhysicsVelocity{ Linear = float3.zero } );
            _manager.SetComponentData( m_entity, new Rotation{ Value = quaternion.EulerXYZ( new float3( 0, 0, 1 ) ) } );
